function PryzmaStartViewModel(model) {
    var self = this;
...
function DoswiadczenieStartViewModel(model) {
    var self = this;

    this.pryzmy = ko.observableArray([]);

    var tmp = [];
    for (var i in model.pryzmy || []) {
        var pryzma = model.pryzmy[i];
        tmp.push(new PryzmaStartViewModel(pryzma));
    }
    this.pryzmy(tmp);

    this.isValid = ko.computed(function() {
        return Enumerable.From(self.pryzmy()).All(function(p) { return p.isValid() });
    });
